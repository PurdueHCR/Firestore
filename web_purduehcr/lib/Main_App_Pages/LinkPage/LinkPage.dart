...
import 'package:purduehcr_web/Main_App_Pages/LinkPage/LinkCreationForm.dart';
import 'package:purduehcr_web/Main_App_Pages/LinkPage/LinkEditForm.dart';
...
class _LinkPageState extends BasePageState<LinkBloc, LinkEvent, LinkState> {
  ...
  Widget buildMobileBody({BuildContext context, LinkState state}) {
    ...
    if (_selectedLink == null) {
      return LinkList(links: _linkBloc.state.links, onPressed: _onPressed);
    } else {
      return BlocProvider(
  ...
  _onChangeState(BuildContext context, LinkState state) {
    if (state is CreateLinkSuccess) {
      ...
      final snackBar = SnackBar(
        ...
        content: Text(
            "Sorry. There was a problem creating your link. Please try again."),
      );
      Future.delayed(Duration(seconds: 1), () {
        Scaffold.of(context).showSnackBar(snackBar);
        _linkBloc.add(LinkDisplayedMessage());
      ...
      final snackBar = SnackBar(
        backgroundColor: Colors.red,
        content: Text(
            "Sorry, there was an error updating your link. Please try again."),
      );
      ...
      Future.delayed(Duration(seconds: 1), () {
        ...
        if (state.enabled != null) state.originalLink.enabled = state.enabled;
