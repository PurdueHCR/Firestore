...
const _DrawerOptions SUBMIT_POINTS = _DrawerOptions(
    "Submit Points", "/submit_points", Icon(Icons.add));
const _DrawerOptions HANDLE_POINTS = _DrawerOptions(
    "Handle Points", "/handle_points", Icon(Icons.message));
const _DrawerOptions MY_POINTS = _DrawerOptions(
    "My Points", "/my_points", Icon(Icons.list));
const _DrawerOptions HISTORY = _DrawerOptions(
    "House History", "/house_history", Icon(Icons.history));
const _DrawerOptions LINKS = _DrawerOptions(
    "Links", "/links", Icon(Icons.link));
const _DrawerOptions CONTROLS = _DrawerOptions(
    "Controls", "/controls", Icon(Icons.build));
const _DrawerOptions POINT_TYPE_CONTROLS = _DrawerOptions(
    "Point Categories", "/point_type_controls", Icon(Icons.list));
const _DrawerOptions HOUSE_CODES = _DrawerOptions(
    "House Codes", "/house_codes", Icon(Icons.home));
const _DrawerOptions FIND_USERS = _DrawerOptions(
    "Find Users", "/find_users", Icon(Icons.search));
const _DrawerOptions REWARDS = _DrawerOptions(
    "Rewards", "/rewards", Icon(Icons.cake));

const List<_DrawerOptions> RESIDENT_LIST = [OVERVIEW, SUBMIT_POINTS, MY_POINTS];
const List<_DrawerOptions> RHP_LIST = [
  OVERVIEW,
  SUBMIT_POINTS,
  MY_POINTS,
  HANDLE_POINTS,
...
const List<_DrawerOptions> PROFESSIONAL_STAFF_LIST = [
  OVERVIEW,
  LINKS,
  HISTORY,
  HOUSE_CODES,
...
const List<_DrawerOptions> PRIVILEGED_USER_LIST = [
  OVERVIEW,
  SUBMIT_POINTS,
  MY_POINTS,
  LINKS
...
class PhcrDrawer extends Drawer {
  ...
  Widget build(BuildContext context) {
    ...
    String permissionName = UserPermissionLevelConverter
        .convertPermissionToString(user.permissionLevel);
    String accountDetails = "";
    if (user.isCompetitionParticipant()) {
      accountDetails =
          user.house + " House: " + user.floorId + " " + permissionName;
    }
    ...
    if (user.isCompetitionParticipant()) {
      ...
      houseIcon = Image.network(BlocProvider
          .of<AuthenticationBloc>(context)
          .state
          .preferences
          .defaultImageURL);
